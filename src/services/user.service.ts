...
import { CreateUserDto, UpdateUserDto, UserDatabaseResponse, UserPasswordResetDto } from "../dtos/user.dto";
import { AppErrorResponse, ClientError, ClientErrorContext } from "../responses/error.response";
...
export class UserService {
     ...
     * @author Akshay Priyadarshi <https://github.com/Akshay-Priyadarshi/>
     */
    getCompleteUserByEmail = async (email: string): Promise<UserDatabaseResponse | null> => {
     ...
     * @author Akshay Priyadarshi <https://github.com/Akshay-Priyadarshi/>
     */
    getCompleteUserById = async (id: string): Promise<UserDatabaseResponse | null> => {
     ...
     * @author Akshay Priyadarshi <https://github.com/Akshay-Priyadarshi/>
     */
    createUser = async (createUserDto: CreateUserDto): Promise<UserDatabaseResponse | null> => {
     ...
     * @author Akshay Priyadarshi <https://github.com/Akshay-Priyadarshi/>
     */
    updateUser = async (id: string, updateUserDto: UpdateUserDto): Promise<UserDatabaseResponse | null | undefined> => {
     ...
     * @author Akshay Priyadarshi <https://github.com/Akshay-Priyadarshi/>
     */
    deleteUser = async (id: string): Promise<UserDatabaseResponse | null | undefined> => {
     ...
     * @author Akshay priyadarshi <https://github.com/Akshay-Priyadarshi/>
     */
    resetUserPassword = async (id: string, passwordResetDto: UserPasswordResetDto): Promise<boolean | undefined> => {
        ...
        if (user) {
            if (verifyPassword(passwordResetDto.old, user.password)) {
                const updateResult = await user.updateOne({ password: passwordResetDto.new }, { runValidators: true });
                if (updateResult.modifiedCount === 1) {
                    return true;
                }
            } else {
     ...
     * @author Akshay Priyadarshi <https://github.com/Akshay-Priyadarshi/>
     */
    verifyUserRedirect = async (signedToken: string): Promise<boolean | undefined> => {
