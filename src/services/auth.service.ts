...
export class AuthService {
     ...
     */
    async login(loginCredentials: ILoginCredentialsDto): Promise<ILoginResponseDto | undefined> {
        const user = await this.userService.getCompleteUserByEmail(loginCredentials.email);
        if (user) {
            if (verifyPassword(loginCredentials.password, user.password)) {
                const payload = { sub: user.id };
                const signedAuthToken = getSignedJwtToken(payload, ENV_ACCESS_TOKEN_SECRET);
     ...
     * @author Akshay Priyadarshi <akshayp1904@outlook.com>
     */
    async signup(createUserDto: CreateUserDto): Promise<UserDatabaseResponse | null | undefined> {
