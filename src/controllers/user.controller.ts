...
export class UserController {
    constructor(private userService = new UserService(), private mailService = new MailService()) {}
    ...
    getAllUsers = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const users = await this.userService.getAllUsers(getPaginationDataFromQuery(req.query));
            const appResponse = new AppResponse({
                reqPath: req.originalUrl,
    ...
    resetUserPassword = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const resetResult = await this.userService.resetUserPassword(req.params.userId, req.body);
            if (resetResult) {
                res.json(
                    new AppResponse({
                        reqPath: req.originalUrl,
    ...
    verifyUser = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const signedVerifyToken = getSignedJwtToken(payload, ENV_VERIFY_USER_SECRET);
    ...
    verifyUserRedirect = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const ifVerified = await this.userService.verifyUserRedirect(req.params.verifyToken);
